...
const app = new Hono<{ Bindings: { DB: D1Database; PUBLIC_KEYS: KVNamespace; TOKEN_MANAGER: DurableObjectNamespace; COST_LEDGER: DurableObjectNamespace; SWARM_RELAY: DurableObjectNamespace; TOKEN_SIGNING_KEY: string; PEN_TEST_MODE?: string } }>();
...
app.post("/tools/execute", async (c) => {
  try {
    ...
    // Execute tool
    const result = await gatewayTools.handle(tool, args);
...
app.post("/sandbox/review", async (c) => {
  try {
    const { skillId } = c.get("validBody") as { skillId: string };
    const result = await gatewayTools.handle("sandbox.review", { skillId });
    return c.json(result);
  } catch (error: any) {
    return c.json({ error: error.message }, 500);
  }
...
app.post("/tools/developer.write_code", async (c) => {
  try {
    ...
    const result = await gatewayTools.handle("developer.write_code", args);
    return c.json(result);
  } catch (error: any) {
    return c.json({ error: error.message }, 500);
  }
...
app.post("/org/create", async (c) => {
  try {
    ...
    const org = orgManagementTools.createOrg(name, plan);
    return c.json(org);
  } catch (error: any) {
    return c.json({ error: error.message }, 500);
  }
...
app.post("/org/:orgId/invite", async (c) => {
  try {
    ...
    const success = orgManagementTools.inviteMember(orgId, email, role);
...
app.delete("/org/:orgId/member/:userId", async (c) => {
  try {
    ...
    const success = orgManagementTools.removeMember(orgId, userId);
    return c.json({ success });
  } catch (error: any) {
    return c.json({ error: error.message }, 500);
  }
...
app.get("/org/:orgId/members", async (c) => {
  try {
    const orgId = c.req.param("orgId");
    const members = orgManagementTools.getMembers(orgId);
...
app.get("/org/:orgId/stats", async (c) => {
  try {
    const orgId = c.req.param("orgId");
    const stats = orgManagementTools.getStats(orgId);
    return c.json(stats);
  } catch (error: any) {
    return c.json({ error: error.message }, 500);
  }
